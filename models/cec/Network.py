...
class MYNET(Net):
    ...
    def estimate_prototype(self, dataloader, class_list, session):
        ...
        for w in range(self.args.low_way):
            fc_weight = F.normalize(self.fc.weight[:class_list.min()+w].clone(), p=2, dim=-1)
    ...
    def _forward(self,support,query,support_label=None,pseudo_proto=None,test=False,data_drop=True):
        if self.args.meta:
            if test:
                output = self.args.temperature * F.linear(F.normalize(support, p=2, dim=-1), F.normalize(self.fc.weight, p=2, dim=-1))
                return output
            else:
                ...
                for w in range(self.args.low_way):
                    ...
                    target_label = support_label[0::self.args.low_way] - num_pseudo_new_classes + rand_num # target label is always 60 + rand_num
                    ...
                    for i in range(self.args.inner_steps):
                        ...
                        #estimated_proto = self.proto_estimator(torch.cat([fc_weight, data], dim=0), inner_params)
                        #cur_fc = torch.cat([fc_weight, estimated_proto.unsqueeze(0)], dim=0)
                        estimated_proto = self.proto_estimator(data, fc_weight[:60], inner_params)
                        cur_fc = torch.cat([fc_weight, estimated_proto.unsqueeze(0)], dim=0)
                    ...
                    #cur_fc = torch.cat([fc_weight, data], dim=0)
                    estimated_proto = self.proto_estimator(data, fc_weight[:60], inner_params)
                    ...
                    adapted_fc.append(estimated_proto)
                ...
                return output, adapted_fc, pseudo_proto_list
...
class ProtoEstimator(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(60,128, bias=False)
    ...
    def forward(self, data, proto_base, params):
        ...
        #x = self.dropout(x)
        x = F.relu(F.linear(x, params['fc2.weight']))
        #x = self.dropout(x)
        estimated_proto = F.linear(x, params['fc3.weight']) + data

        return estimated_proto.mean(dim=0)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0, estimator=False):
        ...
        #self.w_qs2 = nn.Linear(n_head * d_k, n_head * d_k, bias=False)
        #self.w_ks2 = nn.Linear(n_head * d_k, n_head * d_k, bias=False)
        #self.w_vs2 = nn.Linear(n_head * d_v, n_head * d_v, bias=False)
    ...
    def forward(self, q, params=None):
        ...
        if self.estimator:
            ...
            #output = torch.cat([output[:num_plp], output[num_plp:].mean(dim=0).unsqueeze(0)])
            output = output[num_plp:].mean(dim=0)
