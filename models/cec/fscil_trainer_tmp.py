...
class FSCILTrainer(Trainer):
    ...
    def get_base_dataloader_meta(self):
        ...
        if self.args.new_loader:
            sampler = CategoriesSampler(trainset.targets, self.args.train_episode, 20, 10)
            trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_sampler=sampler, num_workers=8,
                                                      pin_memory=True)
            
            if self.args.fullproto:
                new_sampler = CategoriesSampler(trainset.targets, self.args.train_episode, 5+40, self.args.num_fullproto + 45)
            else:
    ...
    def replace_to_rotate(self, proto_tmp, query_tmp, pseudo_tmp, full_tmp=None):
        ...
        num_pseudo_new_classes = int(pseudo_tmp.shape[0] / 45)
            ...
            if self.args.meta_sgd:
                ...
                if self.args.feat_transform:
                    ...
                    optimizer = torch.optim.SGD([{'params': self.model.module.proto_estimator.parameters(), 'lr': self.args.outer_lr},
                                                {'params': self.model.module.fc.parameters(), 'lr': self.args.lr_base},
                                                {'params': self.model.module.encoder.parameters(), 'lr': self.args.lr_base},
                                                {'params': self.model.module.proto_refiner.parameters(), 'lr': self.args.outer_lr}],
                                                momentum=0.9, nesterov=True, weight_decay=self.args.decay)
                    #optimizer = torch.optim.Adam([{'params': self.model.module.proto_estimator.parameters(), 'lr': self.args.outer_lr}],
                    #                            #{'params': self.model.module.encoder.parameters(), 'lr': self.args.lr_base}],
                    #                            )
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    if args.set_no_val or args.meta: # set no validation
                        ...
                        self.model.module.refined_fc.weight.data = self.model.module.fc.weight.data.clone()
    ...
    def base_train(self, model, trainloader, optimizer, scheduler, epoch, args, new_trainloader=None):
        ...
        if self.args.new_loader:
            ...
            query_label = torch.arange(args.episode_way).repeat(40) + args.base_class
            ...
            for i, (batch, batch_new) in enumerate(zip(tqdm_gen, tqdm_gen_new), 1):
                ...
                data_new, true_label_new = [_.cuda() for _ in batch_new]
                
                num_new_classes = 5
                num_pseudo_new_classes = random.randint(0,39)
                index_new = np.tile(np.arange(num_new_classes),145) + np.repeat(np.arange(145)*45,num_new_classes)
                index_pseudo_new = np.tile(np.arange(num_new_classes,num_new_classes+num_pseudo_new_classes),45) + np.repeat(np.arange(45)*45,num_pseudo_new_classes)
                ...
                if self.args.fullproto:
                    proto, query = data_new[:25], data_new[25:225]
                    fulldata = data_new[225:]
                ...
                pseudo_tmp = deepcopy(
                    data_pseudo_new.reshape(45, num_pseudo_new_classes, data_pseudo_new.shape[1], data_pseudo_new.shape[2], data_pseudo_new.shape[3])[:,
                ...
                if num_pseudo_new_classes > 0:
                    ...
                    for jj in range(num_pseudo_new_classes):
                        pseudo_proto.append(pseudo_tmp[jj::num_pseudo_new_classes].mean(dim=0))
                ...
                if self.args.data_dropout:
                    ...
                    logits, estimated_proto, transformed_feats = model.module._forward(support, query, s_l, pseudo_proto)
                    tmp = []
                    for e in estimated_proto:
                        tmp.append(e[-1])
                    estimated_proto_tmp = torch.stack(tmp)
                    ...
                    if self.args.fullproto:
                        ...
                        estimated_proto_tmp = F.normalize(estimated_proto_tmp, p=2, dim=-1)
                    ...
                    if self.args.feat_transform and self.args.fullproto:
                        transform_loss = 0
                        for jj in range(target_proto.shape[0]):
                            transformed_feats[jj] = transformed_feats[jj].mean(dim=0)
                        transformed_feats = torch.stack(transformed_feats)

                        transform_loss = abs(target_proto - transformed_feats).mean()
                    else:
                        transform_loss = 0
                    ...
                    for jj in range(5):
                        if num_pseudo_new_classes > 0:
                            #tmp = [self.model.module.fc.weight[:60], pseudo_proto, estimated_proto[jj].unsqueeze(0)]
                            tmp = estimated_proto[jj]
                        else:
                        ...
                        loss_new = loss_new + F.cross_entropy(output, query_label[0::5] + num_pseudo_new_classes)
                        logits.append(self.args.temperature * F.linear(F.normalize(torch.cat([query[0], query[1][jj::5]], dim=0), p=2, dim=-1), F.normalize(tmp.detach(), p=2, dim=-1)))
                        #loss = loss + F.cross_entropy(output, torch.cat([true_label, query_label[0::5]]))
                    contrastive_loss = F.linear(estimated_proto_tmp, F.normalize(tmp[:60+num_pseudo_new_classes].detach(), p=2, dim=-1)).mean()
                    total_loss = (loss_base + loss_new * self.args.query_new_lamb) / (5+5*self.args.query_new_lamb) - proto_loss * self.args.proto_lamb + transform_loss * self.args.trans_lamb + contrastive_loss * self.args.lamb_contrastive
                    ...
                    #    #tmp = count_acc(logits[jj], torch.cat([true_label, query_label[0::5]]))
                        acc += count_acc(logits[jj], torch.cat([true_label, query_label[0::5]]))
                    acc = acc / 5
